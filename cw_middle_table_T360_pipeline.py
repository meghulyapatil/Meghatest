...
cus_tbl = dbutils.widgets.get('cus_tbl')
cal_tbl=dbutils.widgets.get('cal_tbl')
svc_ls_tbl=dbutils.widgets.get('svc_ls_tbl')

out_tbl_middle=dbutils.widgets.get('out_tbl_middle')
...
# start_dt = '2020-01-06'
# end_dt =   '2020-01-12'
# promo_dt = date(2019, 10, 6)  # '2019-10-06'

# start_dt = date(2019, 12, 2)  # "2019-12-02"
# end_dt = date(2020, 1, 5)  # "2020-01-05"
...
promo_df = (
    ...
    .filter((F.col("country").isin(promo_country))
            & F.col("start_date").between(promo_dt, end_dt))
...
t360_df = (
    ...
    .withColumn("moptransactionind",
                (F.col("customerorderchannelname") == 'MOP').cast('tinyint'))
#     .withColumn("saleschannelcode", F.lit("CO"))
#      .withColumn(
#         "week_level",
#         F.when(F.dayofweek("businessdate").isin(1, 7), F.lit("weekend")).otherwise(
...
store_edw_df = (
    ...
    .select(store_cols)
    .withColumn('StoreTimeZone', F.split(F.col('StoreTimeZoneDescription'), ' ').getItem(1))
)
...
cus_df = (
    ...
    .filter((F.col("loyaltyprogramname").isin(lpn))
        &  (F.col("fiscalweekbegindate") >= start_dt)
        &  (F.col("fiscalweekbegindate") <= end_dt))
...
cal_df = (
      spark.table(cal_tbl)
      .select(cal_cols)
      .filter((F.col("fiscalweekbegindate") >= start_dt)
        &  (F.col("fiscalweekbegindate") <= end_dt))
)
...
mop_local_df = (
    mop_df
    .join(broadcast(store_edw_df), "storenumber", "left")
...
line_item_final_agg_middle_df = (
  ...
  .withColumn(
        "total_modifier_qty",
        F.when(F.col("childlinesequencenumber") > 0, F.col("itemquantity")),
    )
    .withColumn(
        "unpaid_modifier_qty",
        F.when(
            (F.col("childlinesequencenumber") > 0) & (F.col("nds") == 0), F.col("itemquantity")
        ),
    )
    .withColumn(
        "paid_modifier_qty",
    ...
    .withColumn(
        "LS_trans", F.when(F.col("ownership_type") == "LS", F.col("transactionid"))
    )
    .withColumn(
    ...
    .withColumn(
        "USCO_transaction",
        F.when(F.col("ownership_type") == "CO", F.col("transactionid")),
    )
    .withColumn(
        "USCO_MOP_transaction",
        F.when(
            (F.col("ownership_type") == "CO") & (F.col("moptransactionind") == 1),
            F.col("transactionid"),
        ),
    )
    ...
    .agg(
        ...
        F.round(
            ((F.sum("total_modifier_qty") / F.sum("itemquantity")) * 100), 2
        ...
        F.round(
            (F.sum("session_count")) / (F.count("itemid")), 0
        ).alias("session_count"),
        F.round(
            (F.sum("app_session_count")) / (F.count("itemid")), 0
...
cus_cal_final_df = cus_cal_df.join(broadcast(cal_df), "calendardate", "left")
...
tsd_df = t360_lto_final_df.groupBy(groupbytsd).agg(
    F.round(
...
middle_layer_final_df = (
  line_item_final_agg_middle_df
  .join(tsd_city_df, tsd_join_keys, "left")
  .join(broadcast(cus_cal_final_df), lapsed_join_keys, "left")
)
...
middle_layer_final_df.createOrReplaceTempView("my_temp_table2");

spark.sql("create table cma_teams_store360.donotuse_cw_middle_t360_1month as select * from my_temp_table2"); 
